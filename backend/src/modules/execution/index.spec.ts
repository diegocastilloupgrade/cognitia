...
async function createAndStartSession(baseUrl: string): Promise<number> {
...
describe("execution module", { concurrency: 1 }, () => {
  ...
  test("execution runtime-state survives backend restart with the same persistent store", async (t) => {
    await truncateAllTables();
    t.after(() => truncateAllTables());

    const firstServer = await startTestServer();
    const sessionId = await createAndStartSession(firstServer.baseUrl);

    const startItem = await jsonRequest(firstServer.baseUrl, `/api/execution/session/${sessionId}/item/3.1/start`, {
      method: "POST",
      body: JSON.stringify({}),
    });
    assert.equal(startItem.status, 201);

    const firstRuntimeState = await jsonRequest(
      firstServer.baseUrl,
      `/api/execution/session/${sessionId}/runtime-state`
    );
    ...
    const resumedRuntimeState = await jsonRequest(
  ...
  test("finalize-item persists item result consistently and keeps it after restart", async (t) => {
    ...
    assert.equal(
      (((resultsBeforeRestart.body as Array<unknown>)[0] as Record<string, unknown>).data as Record<string, unknown>)
        .stimulusId,
      "mv-01"
    );
    ...
    assert.equal(
      (((resultsAfterRestart.body as Array<unknown>)[0] as Record<string, unknown>).data as Record<string, unknown>)
        .stimulusId,
      "mv-01"
    );
  });
